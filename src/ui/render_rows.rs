...
impl Row {
    ...
    pub fn anchor(&self) -> Option<(Side, u32)> {
        match &self.kind {
            RowKind::Line { kind, old_line, new_line } => match kind {
...
pub fn build_rows(changeset: &Changeset) -> Vec<Row> {
    ...
    for (fi, file) in changeset.files.iter().enumerate() {
        rows.push(Row { file_idx: fi, kind: RowKind::FileHeader, text: file.display_path().to_string() });
        if file.is_binary {
            rows.push(Row { file_idx: fi, kind: RowKind::HunkHeader, text: "Binary file".into() });
            continue;
        }
        for hunk in &file.hunks {
            let header = format!(
                ...
                hunk.new_start,
                hunk.new_count,
                hunk.section.as_ref().map(|s| format!(" {s}")).unwrap_or_default()
            );
            rows.push(Row { file_idx: fi, kind: RowKind::HunkHeader, text: header });
